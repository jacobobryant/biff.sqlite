...
(deftest authorized-write-update-pk-change-test
  (testing "update that changes primary key produces :create (new PK not found in before-snapshot)"
    (let [ctx {:biff.sqlite/read-pool *read-pool*
               :biff.sqlite/write-conn *write-conn*
               :biff.sqlite/columns test-columns
               :biff.sqlite/authorize allow-all}
          diff (biff.sqlite/authorized-write
                ...
                {:update :user
                 :set {:user/id "u1-new"}
                 :where [:= :user/id "u1"]})]
      ;; The read transaction can't match the new PK "u1-new" to the old PK "u1",
      ;; so the diff shows a :create for the new PK with no corresponding :delete.
      (is (= 1 (count diff)))
      (is (= :create (:op (first diff))))
      (is (nil? (:before (first diff))))
      (is (= "u1-new" (-> diff first :after :user/id))))))
